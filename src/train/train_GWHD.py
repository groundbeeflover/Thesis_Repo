...
detector = DGFasterRCNN(
    n_classes=2,
    batchsize=2,
    n_tdomains=tr_dataset.num_domains,
    n_vdomains=vl_dataset.num_domains
)

RUN_DIR = Path("outputs/runs/debug_gwhd")
...
PRETRAINED_CHECKPOINT = Path("checkpoints/pretrained/gwhd_best_prop.ckpt")
BEST_CHECKPOINT = CHECKPOINT_DIR / "best.ckpt"

if PRETRAINED_CHECKPOINT.exists():
    detector.load_state_dict(
        torch.load(PRETRAINED_CHECKPOINT, map_location=device)["state_dict"]
    )
    print(f"Loaded pretrained checkpoint: {PRETRAINED_CHECKPOINT}")
else:
    print(f"Warning: pretrained checkpoint not found: {PRETRAINED_CHECKPOINT}")
...
try:
    ...
    print("Training interrupted. Skipping test phase.")
    raise SystemExit(130)

if BEST_CHECKPOINT.exists():
    detector.load_state_dict(
        torch.load(BEST_CHECKPOINT, map_location=device)["state_dict"]
    )
else:
    print(f"Best checkpoint not found, skipping test: {BEST_CHECKPOINT}")
    raise SystemExit(0)
...
for index, data_sample in enumerate(iter(test_dataloader)):
  ...
  prediction_domain_dir = PREDICTIONS_DIR / str(labels[0].item())
  prediction_domain_dir.mkdir(parents=True, exist_ok=True)

  path = prediction_domain_dir / f"{index}.png"
  cv2.imwrite(str(path), cv2.cvtColor(orig_img[0], cv2.COLOR_RGB2BGR))
  print(index)
...
np.savetxt(RESULTS_DIR / "test_acc.txt", np.array(test_acc))
